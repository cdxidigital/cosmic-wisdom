...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    useUtils: () => ({ cosmic: { getMyProfile: { setData: vi.fn(), invalidate: vi.fn() }, getNatalChart: { setData: vi.fn(), invalidate: vi.fn() }, listFiles: { invalidate: vi.fn() }, listSavedItems: { invalidate: vi.fn() } } }),
    cosmic: {
      getMyProfile: { useQuery: () => ({ data: null }) },
      getNatalChart: { useQuery: () => ({ data: null }) },
      listSavedItems: { useQuery: () => ({ data: [] }) },
      ...
      toggleSavedItem: { useMutation: () => mutation },
    },
  },
}));
...
describe("Home profile-missing journey", () => {
  it("renders the real A–B–C start state without personal data", () => {
    const page = renderToStaticMarkup(<ThemeProvider switchable><Home /></ThemeProvider>);
