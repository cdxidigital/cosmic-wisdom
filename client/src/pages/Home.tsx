...
import { ArrowRight, ArrowUpRight, Check, Menu, Sparkles, UserRound, X } from "lucide-react";
import React, { FormEvent, useEffect, useState } from "react";
import { Link } from "wouter";
import { toast } from "sonner";

const HOME_TITLE = "Cosmic Wisdom: Astrology Readings & Birth Charts";
...
const pathCards = [
  { title: "Natal Chart", label: "Your sky map", copy: "Your calculated Sun, Moon, Rising, houses, and aspects.", href: "#natal" },
  { title: "Numerology", label: "Your number rhythm", copy: "A simple Life Path and personal year from your birthday.", href: "/numerology" },
  { title: "Tarot", label: "A reflective draw", copy: "A clear three-card spread when you want a different question.", href: "/tarot" },
  { title: "Palmistry", label: "A visual guide", copy: "A privacy-first guide to exploring your hand at your own pace.", href: "/palmistry" },
  { title: "Compatibility", label: "A private lens", copy: "A pressure-free prompt for communication, care, and connection.", href: "/compatibility" },
];
...
export default function Home() {
  ...
  const requestNatal = () => {
    if (!isAuthenticated) {
      toast("Sign in to create your private mystic home", { description: "Your birth details and readings are kept inside your member account." });
      startLogin();
      return;
    }
    if (!profile) {
    ...
    <main className="min-h-screen bg-[#F3F0E9] text-[#102936] selection:bg-[#EF5D3F] selection:text-white">
      <header className="sticky top-0 z-40 border-b border-[#102936]/10 bg-[#F3F0E9]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 md:px-9">
          ...
          <nav className="hidden items-center gap-5 md:flex"><button onClick={hasChart ? scrollToNatal : requestNatal} className="font-mono text-[9px] tracking-[.14em] text-[#55707d] hover:text-[#EF5D3F]">NATAL CHART</button><Link href="/numerology" className="font-mono text-[9px] tracking-[.14em] text-[#55707d] hover:text-[#EF5D3F]">NUMEROLOGY</Link><Link href="/tarot" className="font-mono text-[9px] tracking-[.14em] text-[#55707d] hover:text-[#EF5D3F]">TAROT</Link><Link href="/palmistry" className="font-mono text-[9px] tracking-[.14em] text-[#55707d] hover:text-[#EF5D3F]">PALMISTRY</Link><Link href="/compatibility" className="font-mono text-[9px] tracking-[.14em] text-[#55707d] hover:text-[#EF5D3F]">COMPATIBILITY</Link><Button onClick={openProfile} className="h-10 rounded-none bg-[#211B2A] px-4 font-mono text-[9px] tracking-[.13em] text-white hover:bg-[#EF5D3F]">{profile ? "MY PROFILE" : "CREATE PROFILE"}</Button></nav>
          <button onClick={() => setMenuOpen(value => !value)} className="flex h-10 w-10 items-center justify-center border border-[#102936]/15 md:hidden" aria-label="Open menu" aria-expanded={menuOpen}>{menuOpen ? <X size={18} /> : <Menu size={18} />}</button>
        </div>
        {menuOpen && <nav className="border-t border-[#102936]/10 bg-[#F3F0E9] px-5 py-4 md:hidden"><div className="mx-auto grid max-w-7xl gap-2"><button onClick={hasChart ? scrollToNatal : requestNatal} className="flex justify-between border-b border-[#102936]/10 py-3 font-mono text-[10px] tracking-[.14em]">NATAL CHART <ArrowRight size={14} /></button><Link href="/numerology" className="flex justify-between border-b border-[#102936]/10 py-3 font-mono text-[10px] tracking-[.14em]">NUMEROLOGY <ArrowRight size={14} /></Link><Link href="/tarot" className="flex justify-between border-b border-[#102936]/10 py-3 font-mono text-[10px] tracking-[.14em]">TAROT <ArrowRight size={14} /></Link><Link href="/palmistry" className="flex justify-between border-b border-[#102936]/10 py-3 font-mono text-[10px] tracking-[.14em]">PALMISTRY <ArrowRight size={14} /></Link><Link href="/compatibility" className="flex justify-between py-3 font-mono text-[10px] tracking-[.14em]">COMPATIBILITY <ArrowRight size={14} /></Link><Button onClick={openProfile} className="mt-2 rounded-none bg-[#211B2A] font-mono text-[9px] tracking-[.14em] text-white">{profile ? "MY PROFILE" : "CREATE PROFILE"}</Button></div></nav>}
      </header>
